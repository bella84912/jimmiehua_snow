...
1. **開一個專案**
   ...
   ![建立一個新專案 hello-nora-calendar，並在左上角切換過去](images/gcal-1-project.png)

2. **開啟 Google 日曆 API**
   左上選單 →「API 和服務」→「程式庫」→ 搜尋 `Google Calendar API` → 點進去 →「啟用」。

   ![搜尋並啟用 Google Calendar API](images/gcal-2-api.png)
...
3. **設定驗證平台 ＋ 把自己加成測試者**
   ...
   ![點「OAuth 同意畫面」會進到 Google Auth Platform，按「開始」](images/gcal-3-start.png)

   ![在「目標對象 → 測試使用者」把自己的 Gmail 加進去](images/gcal-3-testuser.png)
...
4. **建立金鑰（OAuth 用戶端）**
   ...
   ![到左邊「用戶端」按「建立用戶端」，類型選「桌面應用程式」](images/gcal-4-client.png)
